...
| :--------- | :---------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: |
| **G-M5-1** | KV decode incremental == recompute        | F10 A→N→S loose threshold: $\Delta_{max} \le 10^{-2}$, $\varepsilon_{rel} \le 10^{-4}$, $A \ge 99{,}9\%$, $\Delta_{CE} \le 0{,}02$                                                                      | MATCH / PASS via `dismoen-tools` & `compare.py` pada 64 token @ 2K ctx                                                                                            | **PASS** |
...
| **IT-M5-2**  | KV decode vs recompute                         | Exit 0, F10 PASS loose via `dismoen-tools` & `tools/compare.py`                   | STATUS: MATCH, VERDICT: PASS ($\Delta_{max} \le 10^{-2}$) | **PASS** |
