...
|:---|:---:|:---:|:---|
| $p50$ (Median) | **12.32** | [12.31, 12.34] | Interpolasi Linear Type 7 |
| $p90$ | 12.44 | - | Distribusi Ekor |
| $p95$ | **12.52** | [12.44, 12.63] | Evaluasi Project SLO |
| $p99$ | 12.66 | - | Ekor Ekstrem |
| Min / Max | 12.20 / 13.04 | - | Rentang Penuh |
| Mean (Std) | 12.34 (±0.11) | - | Statistik Agregat |
| **$R_{tail} = p95/p50$** | **1.0163** | [1.0089, 1.0247] | **Project SLO $\le 1{,}35$** |

## 2. Kepatuhan Dynamic RAM Budget ($\mathcal{R}_{RAM} \le 0{,}95$)

- **Puncak Pemakaian Fisik (VmHWM)**: **140500 KiB** (137.21 MiB)
- **Uji Stabilitas Leak**: Run 1 = 140500 KiB, Run 2 = 140896 KiB (Delta 0.28% $\le 5\%$, ✅ LEAK-FREE)

| Tier RAM Budget | Anggaran $M_{budget}$ | $\mathcal{R}_{RAM} = \text{VmHWM}/M_{budget}$ | Batas Maksimum | Status |
|:---|:---:|:---:|:---:|:---:|
| `tier_8gb` | 8.0 GiB | 1.67% | $\le 95\%$ | ✅ PASS |
| `tier_16gb` | 16.0 GiB | 0.84% | $\le 95\%$ | ✅ PASS |
| `tier_32gb` | 32.0 GiB | 0.42% | $\le 95\%$ | ✅ PASS |
| `tier_64gb` | 64.0 GiB | 0.21% | $\le 95\%$ | ✅ PASS |
| `host_active` | 7.7 GiB | 1.74% | $\le 95\%$ | ✅ PASS |

## 3. Asersi Stabilitas Throughput Storage ($E_{BW} \le 5\%$)

- **Laporan Sumber W2b**: `m11_w2_async_overlap.json`
- **Variasi Throughput Maksimum $E_{BW}$**: **2.34%**
- **Kriteria Stabilitas**: $E_{BW} \le 5.0\%$ (✅ TERPENUHI)

## 4. Formal Scorecard Gate G-M11-3

- [x] Ukuran Sampel $N \ge 100$: **PASS** ($N = 100$)
- [x] Project SLO $R_{tail} \le 1.35$: **PASS** ($R_{tail} = 1.0163$)
