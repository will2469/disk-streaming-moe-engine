...
| **G-M11-2** | **Async Overlap & Bandwidth Stability** | $\mathcal{E}_{overlap}(c^*_{system}) \ge 80\%$, $E_{BW} \le 5\%$, $N_{in\_flight} \in [2, 4]$ | $\mathcal{E}_{overlap} = 98.07\%$, $E_{BW} = 3.48\%$, $N_{in\_flight} = 2$ | **[PASS]** |
| **G-M11-3** | **Tail Latency SLO & Dynamic RAM** | $R_{tail} \le 1{{,}}35$ ($N \ge 100$), $\mathcal{R}_{RAM} \le 0{{,}}95$, leak-free | $R_{tail} = 1.1615$, $\text{Max } \mathcal{R}_{RAM} \le 2.9\%$, leak delta $\le 0.7\%$ | **[PASS]** |
