...
def parse_model_config(path: String) raises -> Tuple[ModelConfig, Float32]:
    """Parse model config untuk arsitektur Qwen 3.6 hybrid (SSOT M10).

    Otomatis meng-unwrap text_config bila ada (seperti pada Qwen3.6-35B-A3B).
    Exit 2 (ARCHITECTURE_MISMATCH) bila checkpoint adalah arsitektur legacy.
    Exit 3 (CONFIG_MISMATCH / CONFIG_ERROR) bila dimensi arsitektur tidak sesuai.
    """
    ...
    # Validasi Mismatch Arsitektur (Exit 2) - tolak format legacy
    if (
        ...
        raise Error(
            ...
            " is unsupported legacy architecture, but Qwen3.6 was"
