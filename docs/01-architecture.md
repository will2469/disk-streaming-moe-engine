...
Perubahan terhadap trial — **checkpoint resmi Qwen3.6-35B-A3B sudah tersedia**, sehingga fakta arsitektur berikut diverifikasi terhadap model card resmi dan bobot. Angka performa dan alokasi memori empiris kini telah diukur pada benchmark host (TBM → Measured) sesuai prosedur `docs/milestones/M9-port.md` (§ Performance Baseline, § F2 Port, § TBM → Measured) [R4][R5]:

| Aspek / Metrik                    | Trial (Qwen1.5-MoE)             | Port Teoretis (Qwen3.6-35B-A3B)                                                                                                                                                                  | Measured                                                                       | Run-ID            |
| :-------------------------------- | :------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------- | :---------------- |
| **Routed expert**                 | 60, top-4, inter 1408           | **256, top-8, inter 512** [R4]                                                                                                                                                                   | 256 routed (8 active / tok)                                                    | `M9-ARCH-PORT`    |
| **Shared expert**                 | inter 5632, sigmoid gate        | **1 shared + 8 routed aktif; inter 512** [R4]                                                                                                                                                    | 1 shared (selalu aktif)                                                        | `M9-ARCH-PORT`    |
| **Attention**                     | MHA semua layer                 | **10 Gated Attention + 30 Gated DeltaNet** (40 Transformer Blocks: `10 × (3×(GDN+MoE) + 1×(Gated Attention+MoE))`), GQA 16Q/2KV; jadi $L_{att}=L/4$ tepat; MoE hadir pada seluruh 40 block. [R4] | 10 GatedAttn + 30 GDN verified                                                 | `M9-ARCH-PORT`    |
| **Linear attention**              | tidak ada                       | **Gated DeltaNet** di layer sisanya (M8 prasyarat)                                                                                                                                               | 30 GDN layers verified                                                         | `M9-ARCH-PORT`    |
| **Vocab**                         | 151.936                         | **248.320 (padded)** [R4]                                                                                                                                                                        | 248.320 tokens                                                                 | `M9-ARCH-PORT`    |
| **Total / aktif**                 | 14,3 B / 2,7 B official [R1]    | **35 B / 3 B official** [R4]                                                                                                                                                                     | 35 B / 3 B official                                                            | `M9-ARCH-PORT`    |
| **Ukuran disk aktual**            | 28,63 GB BF16                   | GGUF quant ~13–17 GB (Q3/IQ3) atau BF16 ~70 GB untuk oracle shard                                                                                                                                | **13,5–16,8 GB GGUF** (Q3_K/IQ3_XS) / **71,9 GB BF16** (sharded safetensors)   | `M9-20260918-001` |
| **$W_{res}$ port** (Gate G-M9-2)  | 0,28 GiB nominal / 0,60 GiB cap | $\le 1{,}00\text{ GiB}$ (target F1-Port)                                                                                                                                                         | **0,50 GiB nominal** / **1,00 GiB cap** (VmHWM $\le 4{,}40\text{ GiB}$)        | `M9-20260918-001` |
| **$M_{KV}$/token** (Gate G-M9-4)  | 4.096 B/tok (MHA)               | $10.240\text{ B/tok}$ ($10\text{ KiB/tok}$ GQA BF16) / $128\text{ B/tok}$ mini                                                                                                                   | **10.240 B/tok** (full) / **128 B/tok** (mini, $e_{KV} = 0{,}00\%$)            | `F2-GRID-4096`    |
| **$B_{tok}$ decode + $BW_{eff}$** | 2,07 GB (BF16)                  | 1,066 GB (Q3_K analytical) / Target $\ge 0{,}5\text{ tok/s}$ cold                                                                                                                                | **1,066 GB/tok** / $BW_{eff} \approx 3{,}0\text{ GB/s}$                        | `M9-20260918-001` |
| **$T_{tok}$ & $e_T$**             | ~800 ms/tok                     | 0,218 ms (mini) / ~1,8 s (full cold Q3_K)                                                                                                                                                        | **0,214 ms** (mini) / **$e_T = 2{,}00\%$** ($\le 20\%$)                        | `F5-CALIB-0..4`   |
| **Konteks praktis (RAM 8 GB)**    | $\le 8\text{K}$                 | KV kecil (F2, GQA) — batas nyata = prefill CPU (F4)                                                                                                                                              | **4K fit** ($M_{peak}$ nominal 2,11 GiB / cap 4,40 GiB $\ll 7{,}5\text{ GiB}$) | `M9-20260918-001` |

Detail port: `milestones/M9-port.md`. Prasyarat GDN: `milestones/M8-gdn.md`. Laporan benchmark & kalibrasi: `reports/2026-09-18/M9-port-performance.md` dan `reports/2026-09-18/M9-calibration-f1-f2-f5.md`.
