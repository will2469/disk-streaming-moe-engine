...
- **K2 — Parser hardening**: semua invarian F15 ditegakkan sebelum satu byte data dibaca: batas `header_len` ≤ **100 MB**, jumlah tensor ≤ 100.000, whitelist dtype, nama unik, tidak ada rekursi di parser JSON; alokasi memori hanya setelah ukuran tervalidasi terhadap `filesize` (tidak ada alloc dari angka header mentah). [R7][R8]
- **K3 — Fuzz & negative tests**: corpus 20+ mutasi (header length liar, offset overflow/negatif, dtype asing, truncation di tengah data, duplikat nama, JSON rusak) dijalankan pada setiap perubahan parser; kegagalan = crash/hang/OOM apa pun, bukan hanya exit code.
