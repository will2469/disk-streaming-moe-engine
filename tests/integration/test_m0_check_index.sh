...
"$KIMO" check-index "$ISO/nope.st" 2>/tmp/dismoen-iso-err.txt && { echo "hilang FAIL"; fail=1; } || [ $? -eq 2 ] || { echo "hilang exit FAIL"; fail=1; }
...
"$KIMO" check-index "$DUP"/fixture-0000*.safetensors >/tmp/dismoen-dup-out.txt 2>&1 && { echo "duplikat exit FAIL"; fail=1; } || [ $? -eq 1 ] || { echo "duplikat exit FAIL"; fail=1; }
...
"$KIMO" check-index "$WRG"/fixture-0000*.safetensors >/tmp/dismoen-wrong-out.txt 2>&1 && { echo "salah-tempat exit FAIL"; fail=1; } || [ $? -eq 1 ] || { echo "salah-tempat exit FAIL"; fail=1; }
...
"
"$KIMO" check-index "$IMP/model/fixture-00001-of-00003.safetensors" "$IMP/other/fixture-00002-of-00003.safetensors" "$IMP/model/fixture-00003-of-00003.safetensors" >/tmp/dismoen-impostor-out.txt 2>&1 && { echo "impostor exit FAIL"; fail=1; } || [ $? -eq 1 ] || { echo "impostor exit FAIL"; fail=1; }
...
"$KIMO" check-index "$QTE/evil\".safetensors" >/tmp/dismoen-quote-out.txt 2>&1
python3 -c "
import json
